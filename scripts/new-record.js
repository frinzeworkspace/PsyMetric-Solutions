...
document.addEventListener('DOMContentLoaded', function() {
    function countYesAnswers() {
        ...
        for (let i = 1; i <= 13; i++) {
            ...
            let selectedValue = document.querySelector(`div#${questionId} input[type="radio"]:checked`);
    ...
    function updateQuestion2And3() {
        // Question 2
        let q2Value = document.getElementById('MDQ-2').value;
        let q2AnswerText;
        if (q2Value === 'Yes') {
            ...
            q2AnswerText = ''; // Default text when no option is selected
        }
        document.getElementById('TRI-MDQ-2-translate').innerText = q2AnswerText;
        document.getElementById('MDQ-2-answer').innerText = 'Q2: ' + (q2Value ? q2Value : '-');
        document.getElementById('TRI-MDQ-2-answer').innerText = '"' + (q2Value ? q2Value : '-') + '"';

        // Question 3
        let q3Value = document.getElementById('MDQ-3').value;
        let q3AnswerText;
        if (q3Value === 'Moderate Problem' || q3Value === 'Serious Problem') {
        ...
        document.getElementById('MDQ-3-answer').innerText = 'Q3: ' + (q3Value ? q3Value : '-');
        document.getElementById('TRI-MDQ-3-answer').innerText =  '"' + (q3Value ? q3Value : '-') + '"';
    }

    document.getElementById('MDQsubmitBtn').addEventListener('click', function() {
        // Count the number of "Yes" answers for radio buttons
        let yesCount = countYesAnswers();

        // Update total and translation
        ...
        // Update answers for questions 2 and 3
        updateQuestion2And3();
    });
});
...
window.addEventListener('scroll', function() {
    ...
    // Set a threshold (in pixels) to control how far from the "ready" div the header hides/shows
    var threshold = 0;

    // Check if the ready div is within the viewport with a threshold
    if (readyDivPosition.top < window.innerHeight - threshold && readyDivPosition.bottom >= 0) {
        header.classList.add('hidden'); // Gradually hide the header
    } else {
        header.classList.remove('hidden'); // Gradually show the header
    }
});
